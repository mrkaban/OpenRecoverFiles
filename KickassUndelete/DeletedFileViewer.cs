...
namespace KickassUndelete {
	...
	public partial class DeletedFileViewer : UserControl {
        private const string EMPTY_FILTER_TEXT = "Введите текст фильтра...";
		private Dictionary<FileRecoveryStatus, string> _recoveryDescriptions =
				new Dictionary<FileRecoveryStatus, string>() {
                {FileRecoveryStatus.Unknown,"Неизвестно"},
                {FileRecoveryStatus.Resident,"Отлично"},
                {FileRecoveryStatus.Recoverable,"Хорошо (потеря данных не обнаружена)"},
                {FileRecoveryStatus.MaybeOverwritten,"Расчет..."},
                {FileRecoveryStatus.PartiallyOverwritten,"Плохо (данные частично перезаписаны)"},
                {FileRecoveryStatus.Overwritten,"Плохо (данные полностью перезаписаны)"}};
		...
		public void SetScanButtonFinished() {
			bScan.Enabled = false;
			bScan.Text = "Сканирование завершено!";
			progressBar.Hide();
			bRestoreFiles.Show();
		}
